...
FindDataEntry:
	...
	if(!pDataEntry->pData) {
		...
		if(properties.pOwnerTabStrip) {
			properties.pOwnerTabStrip->Raise_OLESetData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
		}
...
FindDataEntry:
	...
	if(!pDataEntry->pData) {
		if(secondChance) {
			if(properties.pOwnerTabStrip && properties.pOwnerTabStrip->dragDropStatus.IsDragging()) {
				return S_OK;
			} else {
				// format not supported
				return DV_E_FORMATETC;
	...
	if(properties.numberOfItemsToDisplay > 1) {
		...
		if(pEntry) {
			...
			*(pDataEntry->pFormat) = *pFormat;
			...
			if(properties.pOwnerTabStrip) {
				properties.pOwnerTabStrip->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, pFormat->lindex, pFormat->dwAspect);
			}
...
void SourceOLEDataObject::SetOwner(TabStrip* pOwner)
{
	if(properties.pOwnerTabStrip) {
	...
	switch(format.cfFormat) {
		...
		if(pEntry) {
			...
			*(pDataEntry->pFormat) = format;
			...
			if(properties.pOwnerTabStrip) {
				properties.pOwnerTabStrip->Raise_OLEReceivedNewData(static_cast<IOLEDataObject*>(this), formatID, format.lindex, format.dwAspect);
			}
