...
BOOL IsValidTabIndex(int tabIndex, TabStrip* pTabStrip)
{
	...
	HWND hWndTabStrip = NULL;
	if(pTabStrip) {
		OLE_HANDLE h = NULL;
		pTabStrip->get_hWnd(&h);
		hWndTabStrip = static_cast<HWND>(LongToHandle(h));
	}

	return IsValidTabIndex(tabIndex, hWndTabStrip);
}

BOOL IsValidTabIndex(int tabIndex, HWND hWndTabStrip)
{
	if(tabIndex < 0 || !IsWindow(hWndTabStrip)) {
		return FALSE;
	}

	return (tabIndex < static_cast<int>(SendMessage(hWndTabStrip, TCM_GETITEMCOUNT, 0, 0)));
}
